...
export const DEFAULT_KEYBINDINGS: ReadonlyArray<KeybindingRule> = [
  ...
  // cmux parity: ⌘⇧D ("Split down") acts on the visible terminal pane, not only when an
  // xterm holds keyboard focus. Gated on `terminalOpen` (which is also true whenever the
  // terminal is focused) so the chord fires from the composer too, matching cmux where the
  // split shortcut always works on the active pane. The arrow variant below stays
  // `terminalFocus` so Cmd+Shift+Down text selection in the composer is never hijacked.
  { key: "mod+shift+d", command: "terminal.splitDown", when: "terminalOpen" },
...
// The split-down letter chord (⌘⇧D) shipped guarded by `terminalFocus`, so it silently
// did nothing whenever focus sat in the composer with the terminal merely visible. cmux's
// "Split down" acts on the active pane in the workspace, so relax the guard to
// `terminalOpen` (still true whenever the terminal is focused). Matched on command + key +
// exact old guard so the `mod+shift+arrowdown` variant — which must stay `terminalFocus`
// to avoid hijacking Cmd+Shift+Down text selection — is left untouched. Idempotent: once
// relaxed the guard no longer matches the old one.
const SPLIT_DOWN_LETTER_CHORD_KEY = "mod+shift+d";
const OUTDATED_SPLIT_DOWN_TERMINAL_GUARD = "terminalFocus";
const RELAXED_SPLIT_DOWN_TERMINAL_GUARD = "terminalOpen";
...
function migrateOutdatedDefaultKeybindingRule(rule: KeybindingRule): {
  ...
  if (
    rule.command === "terminal.splitDown" &&
    rule.key === SPLIT_DOWN_LETTER_CHORD_KEY &&
    rule.when === OUTDATED_SPLIT_DOWN_TERMINAL_GUARD
  ) {
    return {
      rule: {
        key: rule.key,
        command: rule.command,
        when: RELAXED_SPLIT_DOWN_TERMINAL_GUARD,
      },
      migrated: true,
    };
  }
