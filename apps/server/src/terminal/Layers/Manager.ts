...
const UNMANAGED_AGENT_INPUT_ACTIVITY_GRACE_MS = 120_000;
const UNMANAGED_AGENT_OUTPUT_ACTIVITY_GRACE_MS = 15_000;
const MANAGED_AGENT_RUNNING_STALE_MS = 10_000;
...
/**
 * Infer turns for agent CLIs without lifecycle hooks. Enter starts a turn; once
 * the agent produces output, distinct output keeps it live and a short quiet
 * window returns it to idle. Repeated TUI repaint noise is filtered before
 * lastOutputAt is refreshed.
 */
function isUnmanagedAgentSessionBusy(session: TerminalSessionState, now: number): boolean {
  const lastSubmittedAt = session.lastSubmittedAt ?? 0;
  if (lastSubmittedAt <= 0) return false;
  const lastOutputAt = session.lastOutputAt ?? 0;
  if (lastOutputAt >= lastSubmittedAt) {
    return now - lastOutputAt <= UNMANAGED_AGENT_OUTPUT_ACTIVITY_GRACE_MS;
  }
  return now - lastSubmittedAt <= UNMANAGED_AGENT_INPUT_ACTIVITY_GRACE_MS;
}

// Backstop for a managed agent whose "running" hook never got a matching Stop — e.g. the
// user interrupted a Claude turn (Esc), which leaves managedAgentRunning stuck true so the
// tab keeps spinning forever. A silent Start can also happen during launch, so use the
// hook timestamp when no newer visible output exists. The real Stop hook still clears it
// immediately on a normal finish.
function managedAgentRunningIsStale(session: TerminalSessionState, now: number): boolean {
  const stateUpdatedAt = session.managedAgentStateUpdatedAt ?? 0;
  const lastOutputAt = session.lastOutputAt ?? 0;
  if (lastOutputAt <= 0) {
    return stateUpdatedAt > 0 && now - stateUpdatedAt > MANAGED_AGENT_RUNNING_STALE_MS;
  }
  if (stateUpdatedAt > lastOutputAt) {
    return now - stateUpdatedAt > MANAGED_AGENT_RUNNING_STALE_MS;
  }
  return now - lastOutputAt > PROVIDER_OUTPUT_ACTIVITY_GRACE_MS;
}
...
export const __terminalManagerShellTesting = {
  managedAgentRunningIsStale,
  managedAgentRunningStaleMs: MANAGED_AGENT_RUNNING_STALE_MS,
...
export function inspectSubprocessActivity(
  ...
  for (const child of children) {
    ...
    if (
      (!childAgentKind && !isShellLikeProcessName(child.command)) ||
...
async function checkPosixSubprocessActivityByTreeWalk(
  ...
  const inspectPid = async (parentPid: number): Promise<TerminalSubprocessActivity> => {
    ...
    for (const childPid of childPids) {
      ...
      if (
        (!childAgentKind && !isShellLikeProcessName(command)) ||
        nestedActivity.hasNonProviderSubprocess
      ) {
        hasNonProviderSubprocess = true;
      }
...
function deriveActivityAgentState(session: TerminalSessionState): TerminalActivityState | null {
  if (session.managedAgentObserved) return session.managedAgentState;
  return session.unmanagedAgentRunning ? "running" : null;
}

function agentStateFromHookEvent(
  eventType: Exclude<TerminalAgentHookEventType, "Idle">,
...
export class TerminalManagerRuntime extends EventEmitter<TerminalManagerEvents> {
    ...
    return this.runWithThreadLock(input.threadId, async () => {
      ...
      if (!existing) {
        ...
        const session: TerminalSessionState = {
          ...
          managedAgentStateUpdatedAt: null,
          managedAgentObserved: false,
          unmanagedAgentRunning: false,
  ...
  private async startSession(
    ...
    session.unmanagedAgentRunning = false;
  ...
  private processOutputBatch(session: TerminalSessionState, data: string): void {
    ...
    if (latestHookEvent) {
      const hadManagedAgentObserved = session.managedAgentObserved;
      const hadUnmanagedAgentRunning = session.unmanagedAgentRunning;
      session.managedAgentObserved = true;
      session.unmanagedAgentRunning = false;
      // "Idle" (agent SessionStart) marks the session observed-but-not-working so the
      // launch banner never trips the output-busy heuristic — without the "review"
      // dot a real Stop would show. This mirrors CMUX, where SessionStart → idle.
    ...
    try {
      await Promise.all(
        runningSessions.map(async (session) => {
          ...
          let hasRunningSubprocess = false;
          let unmanagedAgentRunning = false;
          ...
          try {
            ...
            if (session.managedAgentObserved) {
              ...
              // No hooks observed. For a detected agent CLI the agent's own output is
              // NOT a reliable "working" signal: it prints a startup banner, its reply,
              // and idle-TUI repaints while doing nothing, which the output-busy window
              // would latch into a permanent pulsing indicator long after a turn ends.
              // So output alone must not mark an agent running — only a non-provider
              // subprocess (e.g. a build the agent spawned) counts here; real agent
              // activity is reported via hooks (Start/Stop). Plain shells (no provider
              // descendant) keep the process-tree check as their only signal.
              ...
              unmanagedAgentRunning =
                subprocessActivity.hasProviderDescendant &&
                isUnmanagedAgentSessionBusy(session, Date.now());
            }
          } catch (error) {
          ...
          // the grace window and nothing else is running, clear the stale state so the tab
          // stops spinning.
          if (
            liveSession.managedAgentObserved &&
            liveSession.managedAgentRunning &&
            !hasNonProviderSubprocess &&
            managedAgentRunningIsStale(liveSession, Date.now())
          ) {
            liveSession.managedAgentRunning = false;
            if (liveSession.managedAgentState === "running") {
              liveSession.managedAgentState = null;
            }
            liveSession.managedAgentStateUpdatedAt = Date.now();
            hasRunningSubprocess = false;
          }
