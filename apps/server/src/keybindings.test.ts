...
it.layer(NodeServices.layer)("keybindings", (it) => {
  ...
  it.effect("relaxes the ⌘⇧D split-down guard to terminalOpen for cmux parity", () =>
    Effect.gen(function* () {
      const { keybindingsConfigPath } = yield* ServerConfig;
      // A config still carrying the old shipped guard on the split-down letter chord,
      // alongside the arrow variant that must stay gated to terminalFocus.
      yield* writeKeybindingsConfig(keybindingsConfigPath, [
      ...
      const letterChord = configState.keybindings.find(
        (entry) =>
          entry.command === "terminal.splitDown" &&
          entry.shortcut.key === "d" &&
          entry.shortcut.shiftKey,
      );
      // ⌘⇧D now fires whenever the terminal is open, not only when xterm holds focus.
      assert.deepEqual(letterChord?.whenAst, { type: "identifier", name: "terminalOpen" });
      ...
      // The arrow variant keeps its terminalFocus guard so it never hijacks text selection.
      assert.isTrue(
        persisted.some(
          (entry) =>
            entry.key === "mod+shift+arrowdown" &&
            entry.command === "terminal.splitDown" &&
