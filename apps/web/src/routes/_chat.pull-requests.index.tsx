...
// Mirror the chat view's offcanvas dock sizing so the PR detail pane opens at the same
// 50/50-on-open width and lower bound.
const PULL_REQUEST_DOCK_MIN_WIDTH = 26 * 16;
const PULL_REQUEST_DOCK_DEFAULT_WIDTH = "max(28rem, calc(50vw - 8rem))";
...
function PullRequestsRouteView() {
  ...
  return (
    <div className={cn(CHAT_MAIN_VIEWPORT_SHELL_CLASS_NAME, CHAT_MAIN_CONTENT_SURFACE_CLASS_NAME)}>
      <SidebarInset className={CHAT_ROUTE_INSET_SHELL_CLASS_NAME} surfaceClassName="bg-transparent">
      ...
      <RightDock
        state={dockState}
        minWidth={PULL_REQUEST_DOCK_MIN_WIDTH}
        defaultWidth={PULL_REQUEST_DOCK_DEFAULT_WIDTH}
        ...
        renderPane={(pane, context) => (
          <Suspense fallback={<PanelStateMessage>Loading pull request...</PanelStateMessage>}>
            <PullRequestDockPane pane={pane} pollingEnabled={context.isActive} />
          </Suspense>
        )}
      />
    </div>
