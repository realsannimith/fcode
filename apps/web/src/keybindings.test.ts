...
describe("split/new/close terminal shortcuts", () => {
  ...
  it("resolves ⌘⇧D split-down whenever the terminal is open (cmux parity)", () => {
    // Mirrors the relaxed server default: the split-down letter chord fires on the visible
    // terminal regardless of xterm focus, while the arrow variant stays terminal-focused.
    const keybindings = compile([
      {
        shortcut: modShortcut("d", { shiftKey: true }),
        command: "terminal.splitDown",
        whenAst: whenIdentifier("terminalOpen"),
      },
      {
        shortcut: modShortcut("arrowdown", { shiftKey: true }),
        command: "terminal.splitDown",
        whenAst: whenIdentifier("terminalFocus"),
      },
    ]);

    // Terminal open but not focused (e.g. focus in the composer): the chord still splits.
    assert.strictEqual(
      resolveShortcutCommand(event({ key: "d", metaKey: true, shiftKey: true }), keybindings, {
        platform: "MacIntel",
        context: { terminalOpen: true, terminalFocus: false },
      }),
      "terminal.splitDown",
    );
    // Still works when the terminal is focused.
    assert.strictEqual(
      resolveShortcutCommand(event({ key: "d", metaKey: true, shiftKey: true }), keybindings, {
        platform: "MacIntel",
    ...
    // No terminal open: nothing to split, chord does not resolve.
    assert.isNull(
      resolveShortcutCommand(event({ key: "d", metaKey: true, shiftKey: true }), keybindings, {
        platform: "MacIntel",
    ...
    // The arrow variant remains gated on terminalFocus so it never hijacks text selection.
    assert.isNull(
      resolveShortcutCommand(
        event({ key: "arrowdown", metaKey: true, shiftKey: true }),
        keybindings,
        { platform: "MacIntel", context: { terminalOpen: true, terminalFocus: false } },
      ),
    );
  });
