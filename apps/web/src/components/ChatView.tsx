...
export default function ChatView({
  ...
  // Authoritative "a turn is actually in flight" signal: status === "running" AND a
  // non-null activeTurnId (the turn.started/turn.completed lifecycle), matching what the
  // sidebar already trusts. Plain phase === "running" stays true while the session process
  // is merely alive/connected, which kept the working animation on at session start.
  const hasLiveTurn = isSessionRunningTurn(activeThread?.session ?? null);
  hasLiveTurnRef.current = hasLiveTurn;
  const isWorking = hasLiveTurn || isSendBusy || isConnecting || isRevertingCheckpoint;
  // ponytail: animation-only "agent is working" signal. A live turn, or the in-flight
  // send that immediately precedes it. Excludes isConnecting/isRevertingCheckpoint so the
  // loading animation never runs at session start or for local housekeeping — it starts
  // when the agent starts and stops when the agent stops.
  const isAgentWorking = hasLiveTurn || isSendBusy;
