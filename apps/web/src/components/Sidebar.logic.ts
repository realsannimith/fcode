...
export function resolveThreadStatusPill(input: {
  ...
  // Mirrored by the thread status pill derivation.
  ...
  // (status === "running" && activeTurnId != null) instead of re-deriving from
  // latestTurn timing — latestTurn.startedAt is absent in the shell snapshot for
  ...
  if (thread.session?.status === "connecting") {
    return {
      ...
      colorClass: "text-sky-600 dark:text-sky-300/80",
      dotClass: "bg-sky-500 dark:bg-sky-300/80",
      // Connecting is session-start, not the agent working — show a static dot, no spinner.
      pulse: false,
      dismissible: false,
    };
  }
