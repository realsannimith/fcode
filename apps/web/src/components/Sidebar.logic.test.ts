...
describe("resolveThreadStatusPill", () => {
  ...
  it("falls back to working when the thread is actively running without blockers", () => {
    expect(
      resolveThreadStatusPill({
        // "Actively running" means an in-flight turn: status "running" AND a non-null
        // activeTurnId (the authoritative isSessionRunningTurn signal). Status alone is
        // session liveness, not the agent working.
        thread: {
          ...baseThread,
          session: { ...baseThread.session, activeTurnId: TurnId.makeUnsafe("turn-running") },
        },
        hasPendingApprovals: false,
