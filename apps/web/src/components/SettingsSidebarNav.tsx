...
export function SettingsSidebarNav(props: {
  ...
  /** Sections hidden in the current interface mode (e.g. GUI-only sections in terminal mode). */
  hiddenSections?: ReadonlySet<SettingsSectionId>;
}) {
  const { onSelectSection, hiddenSections } = props;
  const [query, setQuery] = useState("");
  const trimmedQuery = query.trim();
  const isSearching = trimmedQuery.length > 0;
  const results = useMemo(
    () =>
      rankSettingsSearchEntries(trimmedQuery, SETTINGS_SEARCH_RESULTS_LIMIT).filter(
        (entry) => !hiddenSections?.has(entry.section),
      ),
    [trimmedQuery, hiddenSections],
  );
        ...
        <nav aria-label="Settings sections" className="flex flex-col">
          {SETTINGS_NAV_GROUPS.map((group) => {
            const items = SETTINGS_NAV_ITEMS.filter(
              (item) => item.group === group.id && !hiddenSections?.has(item.id),
            );
            if (items.length === 0) {
              return null;
            }
